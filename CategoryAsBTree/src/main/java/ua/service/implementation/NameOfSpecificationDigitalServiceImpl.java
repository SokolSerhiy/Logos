...
public class NameOfSpecificationDigitalServiceImpl implements  NameOfSpecificationDigitalService{
	...
	@Override
	public List<NameOfSpecDigitDto> findByCategoryIdDto(int id) {
		return findByCategoryId(id).stream().map(this::mapper).collect(Collectors.toList());
	}
	
	private NameOfSpecDigitDto mapper(NameOfSpecificationDigital digital){
		NameOfSpecDigitDto dto = new NameOfSpecDigitDto();
